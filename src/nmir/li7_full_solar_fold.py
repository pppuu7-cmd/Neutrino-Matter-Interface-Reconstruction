"""B16+MSW fold for externally validated two-state 7Li charged-current response."""
...
from .li7_full_response import li7_two_state_sigma_cm2
...
def _continuum(component: str, flux: float, table, spectrum_path: Path, *, oscillated: bool) -> tuple[float, float]:
    ...
    energy_y: list[float] = []
    for x, w in zip(e, f):
        sigma = li7_two_state_sigma_cm2(x)
...
def li7_two_state_b16_fold(model: str, workdir: str | Path, *, oscillated: bool = True) -> dict[str, object]:
    key = model.upper()
    ...
    pep_rate = fluxes["pep"].flux_cm2_s * pep_pee * li7_two_state_sigma_cm2(pep_e) / SNU
    rows["pep"] = {"rate_snu": pep_rate, "energy_snu_mev": pep_rate * pep_e, "mean_captured_enu_mev": pep_e}

    # The thermally broadened ~862-keV 7Be line can reach only the gs branch;
    # the 429-keV excited branch opens at ~1.291 MeV.
    ...
    return {
        "status": "TWO_STATE_RESPONSE_EXTERNALLY_VALIDATED_ON_B8_SOURCE_AVERAGE",
        ...
        "scope_guard": "Neutrino-only upper bound E_dep,nu<=E_nu; daughter/nuclear-mass energy excluded.",
    }
