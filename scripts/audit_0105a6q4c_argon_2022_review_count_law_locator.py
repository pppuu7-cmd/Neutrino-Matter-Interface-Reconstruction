...
BENCHMARK='NMIR-V2-0105A6Q4C'
PREREG_COMMIT='3e11e1fc9343d10d6c97ed2dacff52cee9e228ba'
PARENT_SHA256='6fde625abdd97db2576004183cc3dae4a029b1a8da6ecf02bb89b65e58185059'
PARENT_CLASS='BLOCKED_0105A6Q4B_2018_PRESENTATION_HAS_NO_COUNT_LAW_CANDIDATE_PAGES'
...
def download(p):
    verify_url(URL)
    req=urllib.request.Request(URL,headers={'User-Agent':'NMIR-0105a6q4c/1.0'})
...
def run(parent,pdf):
    ...
    if h256(pb)!=PARENT_SHA256 or pj.get('classification')!=PARENT_CLASS: raise ValueError('parent q4b identity/class mismatch')
    if pdf.exists(): b=pdf.read_bytes(); final=URL; verify_url(final)
    ...
    cls='PASS_0105A6Q4C_2022_REVIEW_CANDIDATE_PAGES_LOCATED_NONDISCOVERY' if cand else 'BLOCKED_0105A6Q4C_2022_REVIEW_HAS_NO_COUNT_LAW_CANDIDATE_PAGES'
    return {'benchmark':BENCHMARK,'preregistration_commit':PREREG_COMMIT,'parent_q4b_result_sha256':PARENT_SHA256,'source':{'provider':'CERN Indico','speaker':'Jacob Zettlemoyer','title':'Experimental efforts and physics capabilities of Coherent Elastic Neutrino-Nucleus Scattering (CEvNS)','date':'2022-09-08','resolved_url':final,'size':len(b),'sha256':h256(b)},'physical_page_count':len(pp),'candidate_pages':[x['physical_page'] for x in cand],'candidate_records':cand,'page_text_retained':False,'classification':cls,'pseudo_data_generated':False,'likelihood_evaluated':False,'systematic_monte_carlo_preregistration_permission_percent':0,'systematic_monte_carlo_execution_permission_percent':0,'observed_bsm_residual_permission_percent':0,'observed_bsm_residual_inspected':False}
def main():
    a=argparse.ArgumentParser(); a.add_argument('--parent-q4b-result',required=True); a.add_argument('--presentation',required=True); a.add_argument('--output',required=True); a.add_argument('--git-sha',required=True); x=a.parse_args()
    try: r=run(Path(x.parent_q4b_result),Path(x.presentation)); r['git_sha']=x.git_sha
