...
def main():
    ...
    out={'gate':'0105a5b-R1l','git_sha':a.git_sha,'classification':None,'url':URL,'http_status':None,'final_url':None,'content_type':None,'bytes':None,'md5':None,'sha256':None,
         'archive_opened':False,'member_listed':False,'source_text_inspected':False,'barr_nuisance_semantics_inferred':False,'standard_3nu_executed':False,'systematic_monte_carlo_executed':False,'observed_bsm_residual_inspected':False,'observed_bsm_residual_permission_percent':0,'systematic_monte_carlo_execution_permission_percent':0}
    try:
        req=urllib.request.Request(URL,headers={'User-Agent':'NMIR-v2-R1l-byte-acquisition-only'})
        ...
        if out['http_status']==200 and len(raw)>0:
            out['classification']='PASS_0105A5B_R1L_BARR2006_SOURCE_BYTES_ACQUIRED_NONDISCOVERY'
        else:
            out['classification']='INFRASTRUCTURE_FAIL_0105A5B_R1L'
    except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, OSError) as e:
        out['classification']='INFRASTRUCTURE_FAIL_0105A5B_R1L'; out['error']=f'{type(e).__name__}: {e}'
