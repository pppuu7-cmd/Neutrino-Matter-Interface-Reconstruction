...
def classify_text(text: str):
    ...
    fig5_ctx=context(text,'Calculations of $\\Delta N_{\\rm eff}$ for the ``Majorana')
    if fig5_ctx is None:
        fig5_ctx=context(text,"Calculations of $\\Delta N_{\\rm eff}$")
    fig6_ctx=context(text,'Same as Fig.~\\ref{fig:majoranaNeff}')
    if fig6_ctx is None:
        fig6_ctx=context(text,'Same as Fig.')

    checks={
        "planck_phrase": planck_ctx is not None,
        "planck_range_0p3_0p4": bool(planck_ctx and "0.3-0.4" in planck_ctx),
        ...
        "conservative_range_0p3_0p4": bool(conservative_ctx and "0.3-0.4" in conservative_ctx),
        ...
        "green_strongly_disfavored": bool(conservative_ctx and "green region strongly disfavored" in conservative_ctx),
        ...
        "fig6_same_as_fig5": bool(fig6_ctx and "Same as Fig." in fig6_ctx and "Dirac" in fig6_ctx),
    }
...
def audit():
    ...
    with tarfile.open(fileobj=io.BytesIO(raw),mode="r:*") as tf:
        ...
        if fh is None: raise RuntimeError("main TeX unreadable")
        data=fh.read()
    text=data.decode("utf-8",errors="replace")
    result=classify_text(text)
    result.update({"iteration":"0080c","source_url":SOURCE_URL,"source_sha256":SOURCE_SHA256,"main_tex":MAIN_TEX,"main_tex_sha256":sha256(data),"main_tex_bytes":len(data),"guard":"Primary textual semantics only; no vector path extraction, no manual figure reading, no scenario/envelope union, no response scan."})
    return result
