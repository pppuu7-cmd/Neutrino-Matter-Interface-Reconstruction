...
def main():
    ...
    out={'gate':'0105a5b-R1m','git_sha':a.git_sha,'classification':None,'url':URL,'http_status':None,'final_url':None,'content_type':None,'bytes':None,'md5':None,'sha256':None,
    ...
    try:
        req=urllib.request.Request(URL,headers={'User-Agent':'NMIR-v2-R1m-byte-acquisition-only'})
        ...
        if out['http_status']==200 and len(raw)>0:
            out['classification']='PASS_0105A5B_R1M_CSMS2011_SOURCE_BYTES_ACQUIRED_NONDISCOVERY'
        else:
            out['classification']='INFRASTRUCTURE_FAIL_0105A5B_R1M'
    except (urllib.error.URLError, urllib.error.HTTPError, TimeoutError, OSError) as e:
        out['classification']='INFRASTRUCTURE_FAIL_0105A5B_R1M'; out['error']=f'{type(e).__name__}: {e}'
