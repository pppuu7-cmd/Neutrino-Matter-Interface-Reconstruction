...
PASS_SHARD="SHARD_PASS_G9_SOURCE_CENTRIC_FINITE_SOURCE"
BLOCKED="BLOCKED_G9_SOURCE_CENTRIC_FINITE_SOURCE_CONVOLUTION"
SCI_FAIL="SCIENTIFIC_FAIL_G9_SOURCE_CENTRIC_FINITE_SOURCE_INVARIANT"
INFRA="INFRASTRUCTURE_FAIL_G9_0090D"


def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--input",required=True); ap.add_argument("--output",required=True)
    args=ap.parse_args()
    files=sorted(Path(args.input).rglob("g9_0090d_shard_*.json"))
    ...
    if len(files)!=9:
        ...
        if bad_contract or bad_head or len(pairs)!=9:
            ...
            result={**base,"status":BLOCKED,"reason":"one or more fixed sentinel shards missed frozen convergence/control criteria",
                    "blocked_reasons":reasons}
        elif statuses==collections.Counter({PASS_SHARD:9}) and len(rows)==81:
            result={**base,"status":"PASS_G9_SOURCE_CENTRIC_FINITE_SOURCE_SENTINEL_AUTHORITY",
