# NMIR Recovery / Source-of-Truth State

Last reconciled: 2026-09-08
Repository: `pppuu7-cmd/Neutrino-Matter-Interface-Reconstruction`
Protocol: `research/RECOVERY_MANUAL.md`
Funnel: `research/NMIR_FUNNEL.md`
Latest completed immutable record: `research/iterations/0089_g9_frozen_discrete_map_derivative_authority.md`, commit `554a3b6d29804877915dc1fd524c4932c3d3a8b5`.
Current next gate: prospectively preregister 0089a G9 frozen-map piecewise turning-root certification. `NMIR_READINESS: 98%`.

## Mandatory recovery order
Read `RECOVERY_MANUAL.md` -> this file -> `NMIR_FUNNEL.md` -> newest numbered iterations/preregs/amendments -> commits newer than this reconciliation -> relevant queued/in-progress/newly-terminal Actions. Repository and validated GitHub Actions artifacts are authority; green CI alone is never scientific PASS.

## Frozen mission / accounting
NMIR is a discovery funnel, not a material scan. Separate focusing/state control, microscopic interaction probability, detector visibility/amplification and irreversible neutrino-supplied deposited energy. Event gain != interaction gain != neutrino-energy gain. Never multiply unvalidated gains. Stored free energy, pumping, preparation and reset work are not neutrino-supplied power.

## Stable non-BSM frontiers
G2: 0049/0050/0051 transfer/background/nuisance requirements closed; 0063 `BLOCKED_NOT_ACTIONABLE` pending same-configuration measured sub-keV rejection × bulk-NR acceptance.
G3: 0047 `PASS_KLOCAL_EXTENSIVITY`; 0048 absolute-strength stress strongly negative but not theorem; 0065 universal short-range/contact coefficient `OPEN_NOT_CURRENTLY_ACTIONABLE`.
G8: 0068 `BLOCKED_RIOEC_NORMALIZATION_AUTHORITY`.

## G9 authority chain
- 0075 `BLOCKED_G9_PERSISTENT_GEOMETRY`: validated local one-ring branch turns before the frozen finite-blur map is covered.
- 0076 `SCIENTIFIC_FAIL_G9_GLOBAL_MULTIIMAGE_KERNEL`: scan-grid sign-change crossing detection missed narrow caustic preimages; base/refined area disagreed strongly.
- 0077 `BLOCKED_G9_TURNING_POINT_KERNEL`: unresolved derivative sign reversal inside a nominal monotone segment. Numerical-method blocker, not physical no-go.
- 0088 terminal `BLOCKED_G9_ROOT_CERTIFICATION`: independent continuous-density derivative route failed closed at `RuntimeError('composite Simpson convergence exhausted')`; no post-hoc patch allowed.
- 0089 `PASS_G9_FROZEN_DISCRETE_MAP_DERIVATIVE_AUTHORITY`: exact piecewise derivative of the actual frozen trapezoidal `projected_mass_g` map is now validated on all open Model-S radial-knot intervals. This PASS authorizes the derivative evaluator only, not turning roots, monotone partitions, areas, kernels or persistent-source convolution.

## 0089 validated authority
Prospective contract: `research/prereg/0089_g9_frozen_discrete_map_derivative_authority.md`, prereg commit `b64dd44f483e1753ebbcbb60e3f68f06a21f7ad5`.

Hosted result:
- head `aeb51b1499597a75fdf749f38c715ecd2746a15d`;
- run/job `34204310210/101990111998`;
- artifact `10047184672`;
- artifact ZIP SHA256 `2093b05093c81de00d464d7b5b55efe20be4d33ae056c52482ad92406b408a56`;
- raw JSON SHA256 `4093c574c83ee0c2ef4d2f0043a579558af1b008e66324880bc8cf35f259dc2c`;
- dedicated tests `4 passed`;
- Model-S git blob SHA1 `e3a0fad3ff877338aad926dbd0a9a43e6c0a897f`;
- frozen `gravity_extended.py` git blob SHA1 `f8d48fb4eae87ff9c1f98ef543dae27c5f359e9c`.

Scientific checks against the frozen prospective thresholds:
- V1 all 2401 open intervals × q={1/4,1/2,3/4}: 7203 points, max symmetric relative mass discrepancy `1.0515670787751558e-14 <= 5e-13`;
- V2 64 deterministic intervals / 192 high-precision derivative points: max h-vs-h/2 replica discrepancy `1.977759571654202e-11 <= 1e-8`; max analytic-vs-reference discrepancy `7.473825379514015e-12 <= 1e-8`;
- V3 all three preregistered toy profiles PASS;
- V4 2401 positive fixed knots checked, zero failures; exact knots rejected as derivative points as frozen.

Artifact ZIP and inner JSON were independently downloaded and hash-verified after the hosted run. This is a scientific PASS, not inference from green CI.

## BSM / B-L authority
0069 unlocks constraint ledger only. BSM response/enhancement remains LOCKED pending reproducible external envelope + formal unlock. Global 0071 target remains `1e-6 eV <= m_V <= 10 GeV` and requires applicable primary families: low-energy neutrino scattering, collider/fixed-target, stellar/SN, cosmology, fifth-force/EP.

Accepted/scoped authority:
- 0078c solar CEvNS B-L excluded area `12.51987545543835 decade^2`.
- 0079/0079a long-range fifth-force asymptotic `g_BL<2.5437058144595744e-25`, conservative `2.5e-25` at 2sigma; no finite-mass continuation.
- 0080d hard-CMB excluded areas Majorana `48.332188584322665`, Dirac `45.129364149717375 decade^2`; alternatives remain separate.
- 0081 signed BBN criterion `Delta Y_p=0.008` at 95% C.L.; 0081a still blocks BBN vector calibration semantics.
- 0084b Shin–Yun SN1987A analytical geometry PASS.
- 0085 Wagner identity-free strongest-published 95% CL upper-limit support `1e-6 <= m_V <= 6.845530367110015e-6 eV`, not a statistical combination.
- 0086/0086a restore collider/fixed-target authority; NA64 is mass-disjoint only from the 0087b low-mass candidate.
- 0074c combined COHERENT likelihood remains blocked while 0074a Ar and 0074b CsI benchmarks remain PASS.

## 0087 low-mass topology/completeness front
0087/0087a establish disconnected partial-authority topology only; no global allowed-region claim. 0087b threat-ranking target is `6.845530367110015e-6 .. 1.4057345497828417 eV`; NA64 is mass-disjoint; known BBN overlap is only `1.0 .. 1.4057345497828417 eV`, `2.7840644564136232%` target log-width. 0087c Cerdeño mass support BLOCKED; 0087d COHERENT mass support BLOCKED; 0087e finite-mass fifth-force support BLOCKED; 0087f BBN tail actionability BLOCKED; 0087g `BLOCKED_RESTRICTED_BELOW_1EV_COMPLETENESS`. BSM remains LOCKED.

## Exact next action
The B-L completeness route is source-authority blocked; G2/G3/G8 are currently non-actionable. G9 is again the highest-value executable front because 0089 removed the specific derivative-authority ambiguity without changing the frozen map.

Next gate must be a separate prospective 0089a **root-certification-only** contract using the exact 0089 derivative. Treat every one of the 2401 open Model-S radial intervals as a source-fixed piece and every exact knot as a fixed boundary. Freeze deterministic interior sampling/root-isolation rules, one-sided boundary-sign semantics, independent original-map finite-difference confirmation, root uniqueness/stability tolerances, and scientific-vs-infrastructure taxonomy before inspecting any derivative sign/root positions. Do not compute accepted areas, one-ring/kernel metrics, finite-source or persistent-source convolution in 0089a. Only a root-certification PASS may authorize a later kernel gate.

## Critical guards
No raster/manual contour reading. No generic dark-photon epsilon contour imported as B-L absent exact complete mapping. No visible/invisible or RHN scenario union. No Majorana/Dirac union/intersection. No T/L union. No finite-mass extrapolation of 0079a. No arbitrary Yukawa threshold. No Wagner statistical-combination claim. No COHERENT benchmark interpolation. No lowering 0081a criteria. No blocked-family-as-null assumption. No cross-family B-L union or BSM response scan before dedicated composition/completeness PASS and formal unlock. No post-hoc patch of 0077 or 0088, no result-selected turning nodes, and no relaxation of frozen G9 validation thresholds.
